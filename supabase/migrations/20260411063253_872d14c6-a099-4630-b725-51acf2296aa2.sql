...
-- Add type column to hashtag_activity
ALTER TABLE public.hashtag_activity 
ADD COLUMN IF NOT EXISTS type text NOT NULL DEFAULT 'hashtag';

-- Create index for type
CREATE INDEX IF NOT EXISTS idx_hashtag_activity_type ON public.hashtag_activity (type);

-- Update the extraction trigger to capture both hashtags and keywords
...
DECLARE
  ...
  stop_words text[] := ARRAY[
    ...
    'na','ya','wa','ni','kwa','la','za','au','na','si','ndi','pia','sana','tu','hii','hizi','yake',
...
BEGIN
  ...
  -- Extract significant keywords (3+ chars, not stop words, not hashtags)
  -- Remove hashtags from text first to avoid duplicates
...
-- Update trending function to return type info
...
AS $function$
  ...
  WHERE ha.created_at > now() - (hours_window || ' hours')::interval
    AND (
      -- hashtags always qualify
      ha.type = 'hashtag'
      -- keywords need at least 2 distinct posts to trend
